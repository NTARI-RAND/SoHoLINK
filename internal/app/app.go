...
import (
	...
	"github.com/NetworkTheoryAppliedResearchInstitute/soholink/internal/compliance"
	...
	"github.com/NetworkTheoryAppliedResearchInstitute/soholink/internal/notification"
...
type App struct {
	...
	OfflineSettler  *payment.OfflineSettler
	PayoutSettler   *payment.PayoutSettler
...
func New(cfg *config.Config) (*App, error) {
	...
	// Write app logs to a file so they survive on windowsgui builds (no console).
	// Fall back to stderr if the file cannot be opened.
	logOut := os.Stderr
	if cfg.Storage.BasePath != "" {
		appLogPath := cfg.Storage.BasePath + string(os.PathSeparator) + "app.log"
		if lf, err := os.OpenFile(appLogPath, os.O_CREATE|os.O_WRONLY|os.O_APPEND, 0640); err == nil {
			logOut = lf
			// Keep a reference so the OS doesn't GC the handle; it lives until process exit.
			_ = lf
		}
	}
	logging.Init(logFormat, logLevel, logOut)
...
func (a *App) initResourceSharing(cfg *config.Config) error {
	...
	// Build settlers now (needs processors + ledger); they are started in
	// startResourceSharing once the long-lived context is available.
	a.OfflineSettler = payment.NewOfflineSettler(a.Store, processors, 10*time.Minute, 1000)

	autoThreshold := int64(0)
	if thStr := os.Getenv("SOHOLINK_AUTO_PAYOUT_THRESHOLD_SATS"); thStr != "" {
		if parsed, pErr := strconv.ParseInt(thStr, 10, 64); pErr == nil && parsed > 0 {
			autoThreshold = parsed
		}
	}
	a.PayoutSettler = payment.NewPayoutSettler(
		a.Store, a.PaymentLedger,
		cfg.Node.DID,
		10*time.Minute,
		autoThreshold,
		os.Getenv("SOHOLINK_AUTO_PAYOUT_PROCESSOR"),
	)
	...
	// Wire TLS — auto-generate self-signed cert if none provided.
	// Set SOHOLINK_INSECURE_NO_TLS=1 to disable TLS entirely (e.g. when running
	// behind a reverse proxy / Cloudflare Tunnel that handles TLS externally).
	if os.Getenv("SOHOLINK_INSECURE_NO_TLS") == "1" {
		log.Printf("[app] TLS disabled via SOHOLINK_INSECURE_NO_TLS — serving plain HTTP (reverse-proxy mode)")
		a.HTTPAPIServer.SetTLSConfig("", "")
	} else {
		certFile, keyFile, autoGenerated, tlsErr := httpapi.EnsureTLSCert(
			cfg.ResourceSharing.TLSCertFile,
			cfg.ResourceSharing.TLSKeyFile,
			cfg.Storage.BasePath,
		)
		if tlsErr != nil {
			log.Printf("[WARN] TLS auto-cert failed: %v — falling back to plain HTTP", tlsErr)
		} else if autoGenerated {
			log.Printf("[app] TLS: auto-generated self-signed certificate for development")
			log.Printf("[app] TLS: for production, set resource_sharing.tls_cert_file and tls_key_file")
			log.Printf("[app] TLS: or set SOHOLINK_INSECURE_NO_TLS=1 to run behind a reverse proxy")
		}
		a.HTTPAPIServer.SetTLSConfig(certFile, keyFile)
	}
	...
	// FCM notifier for Android background wakeup pushes (optional).
	// Set SOHOLINK_FCM_PROJECT_ID and SOHOLINK_FCM_SERVICE_ACCOUNT_JSON env vars
	// to enable. When absent the notifier is nil and the path is a no-op.
	if projectID := os.Getenv("SOHOLINK_FCM_PROJECT_ID"); projectID != "" {
		if saPath := os.Getenv("SOHOLINK_FCM_SERVICE_ACCOUNT_JSON"); saPath != "" {
			saBytes, readErr := os.ReadFile(saPath)
			if readErr != nil {
				log.Printf("[app] FCM notifier init error (push disabled): fcm: read service account file: %v", readErr)
				saBytes = nil
			}
			var saJSON string
			if saBytes != nil {
				saJSON = string(saBytes)
			}
			fcmNotifier, fcmErr := notification.NewFCMNotifier(notification.FCMConfig{
				ProjectID:          projectID,
				ServiceAccountJSON: saJSON,
			})
			if fcmErr != nil {
				log.Printf("[app] FCM notifier init error (push disabled): %v", fcmErr)
			} else {
				a.HTTPAPIServer.SetFCMNotifier(fcmNotifier)
				if a.FedScheduler != nil {
					a.FedScheduler.SetFCMNotifier(fcmNotifier)
				}
				log.Printf("[app] FCM notifier initialized (project=%s)", projectID)
			}
		}
	}

	// Compliance manager (Phase 2/3) — reads the node's Ed25519 signing key,
	// creates the manager, and wires it to the HTTP server.
	// Also wires the platform-specific TPM attester for hardware attestation.
	if a.HTTPAPIServer != nil {
		var privKey ed25519.PrivateKey
		if keyData, err := os.ReadFile(cfg.NodeKeyPath()); err == nil && len(keyData) == ed25519.PrivateKeySize {
			privKey = ed25519.PrivateKey(keyData)
		}
		if privKey != nil && cfg.Node.DID != "" {
			complianceMgr := compliance.NewManager(a.Store, privKey, cfg.Node.DID)
			a.HTTPAPIServer.SetComplianceManager(complianceMgr)
			log.Printf("[app] compliance manager initialized (node=%s)", cfg.Node.DID)
		} else {
			log.Printf("[app] compliance manager skipped: node key or DID not configured")
		}

		// TPM attester — no-op on platforms without a TPM 2.0 device.
		tpmAttester := tpm.NewAttester()
		a.HTTPAPIServer.SetTPMAttester(tpmAttester)
		if tpmAttester.Available() {
			log.Printf("[app] TPM 2.0 attester available")
		} else {
			log.Printf("[app] TPM 2.0 not available — software attestation only")
		}
	}
...
func (a *App) startResourceSharing(ctx context.Context) {
	...
	// Start payment settlers.
	if a.OfflineSettler != nil {
		go a.OfflineSettler.Run(ctx)
		log.Printf("[app]   OfflineSettler: buyer-side charge retry started (interval=10m)")
	}
	if a.PayoutSettler != nil {
		go a.PayoutSettler.Run(ctx)
		threshold := a.PayoutSettler.AutoPayoutThresholdSats
		if threshold > 0 {
			log.Printf("[app]   PayoutSettler: payout retry + auto-payout started (threshold=%d sats)", threshold)
		} else {
			log.Printf("[app]   PayoutSettler: payout retry started (auto-payout disabled)")
		}
	}
